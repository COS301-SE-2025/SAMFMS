...
from schemas.entities import ScheduledTrip, RouteInfo, RouteBounds, TripStatus
...
class SmartTripService:
    ...
    async def create_smart_trip(self, scheduled_trip: ScheduledTrip) -> Dict[str, List[Dict[str, Any]]]:
        origin_lat = scheduled_trip.origin.location.coordinates[1]
        origin_lng = scheduled_trip.origin.location.coordinates[0]
        dest_lat = scheduled_trip.destination.location.coordinates[1]
        dest_lng = scheduled_trip.destination.location.coordinates[0]
        ...
        for i in range(num_samples + 1):
            ...
            if route:
                ...
                if total_duration < min_duration:
                    min_duration, best_start, best_route = total_duration, test_start, route
        ...
        for loc in vehicle_locations:
            ...
            if dist < min_dist:
                min_dist, closest_vehicle_id = dist, loc["vehicle_id"]

        if closest_vehicle_id:
            vehicle_doc = await self.db_management.vehicles.find_one({"_id": ObjectId(closest_vehicle_id)})
            if vehicle_doc:
                closest_vehicle_name = vehicle_doc.get("name", closest_vehicle_name)

        # Driver assignment
        stats = await driver_analytics_service.get_driver_trip_stats("year")
        best_driver_id, best_driver_name, best_rate = None, "Unknown", -1.0
        for stat in stats:
            completed, cancelled = stat["completed_trips"], stat["cancelled_trips"]
            total = completed + cancelled
            rate = completed / total if total > 0 else 0.0
            if rate > best_rate:
                best_rate, best_driver_id, best_driver_name = rate, stat["_id"], stat["driver_name"]

        optimized_start = best_start
        optimized_end = best_start + timedelta(seconds=min_duration)

        # Calculate benefits
        original_duration_min = scheduled_trip.estimated_duration if hasattr(scheduled_trip, "estimated_duration") \
            else (scheduled_trip.route_info.duration / 60 if scheduled_trip.route_info else duration_min)
        ...
        create_request = CreateTripRequest(
            ...
            waypoints=scheduled_trip.waypoints,
            route_info=route_info,
            priority=scheduled_trip.priority,
            constraints=[]
        )
        actual_trip = await trip_service.create_trip(create_request, scheduled_trip.created_by)

        await trip_service.update_trip(actual_trip.id, UpdateTripRequest(driver_assignment=best_driver_id, vehicle_id=closest_vehicle_id))
        await self.db.trips_scheduled.update_one({"_id": ObjectId(scheduled_trip.id)}, {"$set": {"status": TripStatus.COMPLETED.value}})

        # Final JSON for frontend
        smart_id = f"smart-{scheduled_trip.id[:8]}"
        return {
            "data": [{
                "id": smart_id,
                "tripId": scheduled_trip.id,
                "tripName": scheduled_trip.name,
                "originalSchedule": {
                    "startTime": scheduled_trip.start_time_window.isoformat(),
                    "endTime": scheduled_trip.end_time_window.isoformat(),
                    "vehicle": None,
                    "driver": None
                },
                "optimizedSchedule": {
