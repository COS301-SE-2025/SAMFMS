"""
Trips Service main application with trip planning
"""
import asyncio
import logging
import os
...
async def register_with_core_service():
    ...
    try:
        ...
        service_info = {
            "name": "trips",
            "host": os.getenv("TRIPS_HOST","trips"),
            "port": int(os.getenv("TRIPS_PORT", "8000")),
            "version": "1.0.0",
            "protocol": "http",
            "health_check_url": "/health",
            "tags": ["trips", "trip_planning", "router_generation"],
            "metadata": {
                "features": [
                    "trip_planning",
                    "route_generation"
...
async def lifespan(app: FastAPI):
    ...
    logger.info("Trips Service Starting Up...")
    ...
    try:
        ...
        # Register with Core's service directory
...
async def enhanced_background_tasks():
    """Enhanced background tasks for Trips service"""
    while True:
        try:
            ...
            if not db_manager.is_connected():
                ...
                continue
            # Still have to add functionality as needed
...
app = FastAPI(
    title="Trips Service",
    description="Trip planning and route generation services",
    version="1.0.0",
    docs_url="/docs",
    redoc_url="/redoc",
    lifespan=lifespan
...
async def api_documentation():
    ...
    return ResponseBuilder.success(
        data={
            "service": "SAMFMS Trops Service",
            "version": "1.0.0",
            "description": "Will still add text.. for SAMFMS Fleet Management System",
            "base_url": "/gps",
            "endpoints": {
                "locations": {
...
if __name__ == "__main__":
    ...
    port = int(os.getenv("TRIPS_PORT", "8000"))  # Use TRIPS_PORT from environment
    log_level = os.getenv("LOG_LEVEL", "info").lower()
    
    logger.info(f"Starting Trips Service on {host}:{port}")
