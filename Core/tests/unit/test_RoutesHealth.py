...
def _stub_rabbitmq_ok(monkeypatch):
    ...
    async def connect_robust(url): return Conn()
...
def _stub_rabbitmq_fail(monkeypatch, exc=RuntimeError("boom")):
    ...
    monkeypatch.setitem(sys.modules, "rabbitmq.admin", admin)
    aio = ModuleType("aio_pika")
    async def connect_robust(url): raise exc
    aio.connect_robust = connect_robust
    monkeypatch.setitem(sys.modules, "aio_pika", aio)
...
def _stub_service_discovery_ok(monkeypatch, services=None):
    ...
    class SD:
        async def get_healthy_services(self):
            return services
    async def get_service_discovery(): return SD()
...
def _stub_service_discovery_fail(monkeypatch, exc=RuntimeError("sd down")):
    ...
    async def get_service_discovery():
        class SD:
            async def get_healthy_services(self): raise exc
        return SD()
    sd.get_service_discovery = get_service_discovery
    monkeypatch.setitem(sys.modules, "common.service_discovery", sd)
...
def _patch_tracer(
    monkeypatch, *, recent=None, stats=None, summary=None,
...
def _stub_request_router(monkeypatch, *, wait_raises: Optional[Exception] = None):
    ...
    class RespMgr:
        async def wait_for_ready(self):
            if wait_raises: raise wait_raises
            return None
    class RR: response_manager = RespMgr()
...
def test_health_basic_ok(client):
    r = client.get("/health/")
    j = r.json()
    assert r.status_code == 200
    assert j["status"] == "healthy"
    assert j.get("service") in {"samfms-core", j.get("service")}
...
def test_detailed_all_ok(client, monkeypatch):
...
def test_detailed_db_unhealthy_status(client, monkeypatch):
...
def test_detailed_db_raises(client, monkeypatch):
...
def test_detailed_rabbitmq_fails(client, monkeypatch):
...
def test_detailed_service_discovery_degraded_but_overall_ok(client, monkeypatch):
    _stub_database_module(monkeypatch)
    _stub_rabbitmq_ok(monkeypatch)
...
def test_detailed_circuit_breakers_degraded_makes_overall_unhealthy(client, monkeypatch):
    _stub_database_module(monkeypatch)
    _stub_rabbitmq_ok(monkeypatch)
    _stub_service_discovery_ok(monkeypatch)
...
def test_detailed_circuit_breakers_unknown_does_not_flip_overall(client, monkeypatch):
    _stub_database_module(monkeypatch)
    _stub_rabbitmq_ok(monkeypatch)
    _stub_service_discovery_ok(monkeypatch)
...
def test_detailed_deduplicator_raises_makes_unhealthy(client, monkeypatch):
    _stub_database_module(monkeypatch)
    _stub_rabbitmq_ok(monkeypatch)
    _stub_service_discovery_ok(monkeypatch)
    _patch_cb(monkeypatch, states={})
...
def test_ready_response_manager_not_ready(client, monkeypatch):
...
def test_ready_database_not_connected(client, monkeypatch):
...
def test_ready_database_error(client, monkeypatch):
    _stub_request_router(monkeypatch)
    mod = ModuleType("database")
    async def get_database_manager():
...
def test_circuit_breaker_status_ok(client, monkeypatch):
    _patch_cb(monkeypatch, states={"x": {"state": "closed"}})
    r = client.get("/health/circuit-breakers")
    assert r.status_code == 200
    assert "circuit_breakers" in r.json()

def test_circuit_breaker_status_error(client, monkeypatch):
    _patch_cb_raises(monkeypatch)
    r = client.get("/health/circuit-breakers")
...
def test_metrics_with_psutil(client, monkeypatch):
    ...
    class _Proc:
        ...
        def memory_percent(self): return 12.34
    def _Process(): return _Proc()
    ps.Process = _Process
...
def test_metrics_without_psutil(client, monkeypatch):
    _patch_dedup(monkeypatch)
    _patch_cb(monkeypatch, states={})
...
def test_metrics_outer_error(client, monkeypatch):
    _patch_dedup(monkeypatch)
    _patch_cb(monkeypatch, states={})
...
def test_traces_ok(client, monkeypatch):
    _patch_tracer(monkeypatch, recent=[{"id": "1"}, {"id": "2"}])
    r = client.get("/health/traces?limit=2")
    j = r.json()
    assert r.status_code == 200
    assert j["count"] == 2

def test_traces_error(client, monkeypatch):
    _patch_tracer(monkeypatch, raise_recent=RuntimeError("boom"))
    r = client.get("/health/traces")
    assert r.status_code == 500

# ---------- /health/traces/{id} ----------

def test_trace_details_found(client, monkeypatch):
    _patch_tracer(monkeypatch, summary={"id": "abc"})
    r = client.get("/health/traces/abc")
    j = r.json()
    assert r.status_code == 200
    assert j["trace"]["id"] == "abc"

def test_trace_details_not_found(client, monkeypatch):
    _patch_tracer(monkeypatch, summary=None)
    r = client.get("/health/traces/unknown")
    assert r.status_code == 404

def test_trace_details_error(client, monkeypatch):
...
def test_healthy_services_mixed(client, monkeypatch):
    ...
    r = client.get("/health/healthy-services")
    j = r.json()
    assert r.status_code == 200
    s = j["sblocks"]
