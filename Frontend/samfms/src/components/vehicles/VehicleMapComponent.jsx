...
import { Search, MapPin, Navigation, Car, Shield } from 'lucide-react';
...
const VehicleMapComponent = () => {
  ...
  const loadVehicles = useCallback(async () => {
    try {
      ...
      // Transform and filter vehicles with coordinates
      const transformedVehicles = vehiclesData
        .map(vehicle => {
          const vehicleLocation = locationMap[vehicle.id] || locationMap[vehicle.vehicle_id];
          return {
            id: vehicle.id || vehicle.vehicle_id,
            name: vehicle.vehicle_name || vehicle.name || `Vehicle ${vehicle.id}`,
            make: vehicle.make || 'Unknown',
            model: vehicle.model || 'Unknown',
            status: vehicle.status || 'unknown',
            license_plate: vehicle.license_plate || 'N/A',
            // Use actual GPS coordinates or generate mock ones for demo
        ...
        .filter(
          vehicle => vehicle.coordinates && vehicle.coordinates.lat && vehicle.coordinates.lng
        );
  ...
  return (
    <div className="bg-card rounded-md shadow-md mb-6">
          ...
          <div className="flex-1 overflow-y-auto space-y-2">
            {filteredItems.length === 0 ? (
              ...
              filteredItems.map(item => (
                <div
                  key={item.id}
                  className={`p-3 border border-border rounded-md cursor-pointer transition-colors hover:bg-accent ${
